    ...
    if isa(obj1, 'var_symm')
        ...
        if isa(obj2, 'double')
            ...
            if isscalar(obj2)
                ...
                info.num_constr = 0.5*obj1.blkorg{2}*(obj1.blkorg{2}+1);
        ...
        elseif isa(obj2, 'var_sdp')
            ...
            info.active_block = [obj1.block_no, obj2.block_no]; % Since their block_no must be different
            [idx_i,idx_j]=find(triu(ones(obj1.blkorg{2},obj1.blkorg{3}))>0);
            idx_temp = idx_i+obj1.blkorg{2}*(idx_j-1);
            dim_temp = 0.5*obj1.blkorg{2}*(obj1.blkorg{2}+1);
            matrix_temp = sparse(idx_temp,[1:1:dim_temp],2,obj1.blk{2},dim_temp);
        ...
        elseif isa(obj2, 'var_free')
            error('Cannot use ''=='' symmetric and asymmetric variables.');
        end
    elseif isa(obj1, 'double')
        ...
        if isscalar(obj1)
            if isa(obj2, 'var_symm')
                ...
                info.num_constr = 0.5*obj2.blkorg{2}*(obj2.blkorg{2}+1);
                constr_obj = constraint(info);
                return;
            end
        else
