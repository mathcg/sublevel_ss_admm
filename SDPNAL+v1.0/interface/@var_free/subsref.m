%%**********************************************************************
%% Overload bulit-in function 'subsref'
%% 
%% SDPNAL+: 
%% Copyright (c) 2017 by
...
function obj = subsref(var_obj, ind)
    k = length(ind);
    % handle the case for composition: for example: X.info.prob.block;
    if k > 1
        obj = copy(var_obj);
        for i=1:1:k
