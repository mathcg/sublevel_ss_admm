    ...
    if isa(obj1, 'var_sdp')
        ...
        elseif isa(obj2, 'expression')
            ...
            for i = 1:1:length(obj2.active_block)
                block_no = obj2.active_block(i);
                if ismember(block_no, info.active_block)
                    info.Operator_Matrix{block_no} = info.Operator_Matrix{block_no} - obj2.Operator_Matrix{block_no};
                else
                    info.Operator_Matrix{block_no} = -obj2.Operator_Matrix{block_no};
                    info.active_block = union(info.active_block, block_no);
                end
            end
