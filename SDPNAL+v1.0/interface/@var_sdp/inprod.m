...
%% C is a constant matrix or vector, X is a declared variable
%% SDPNAL+: 
%% Copyright (c) 2017 by
...
function exp_obj = inprod(obj1, obj2)
    if isa(obj1, 'var_sdp')
        ...
        if isa(obj2, 'double')
            ...
            info.Operator_Matrix{obj1.block_no} = svec(obj1.blk,0.5*(obj2+obj2'),1);
    ...
    elseif isa(obj2, 'var_sdp')
        ...
        if isa(obj1, 'double')
            ...
            info.Operator_Matrix{obj2.block_no} = svec(obj2.blk,0.5*(obj1+obj1'),1);
