...
function obj = subsref(var_obj, ind)
    k = length(ind);
    % handle multiple calls like : X.model.sdp.block;
    if k > 1
        obj = copy(var_obj);
        for i=1:1:k
    ...
    elseif num_dim == 2
        ...
        if max(ind.subs{1})<=var_obj.blkorg{2} && max(ind.subs{2})<=var_obj.blkorg{3}
            ...
            value_temp = (sqrt(2)/2)*ones(num_constr,1);
            value_temp(ind.subs{1}==ind.subs{2}) = 1;
            idx_i_temp = min(ind.subs{1},ind.subs{2});
            idx_j_temp = max(ind.subs{1},ind.subs{2});
            dim_temp = 0.5*var_obj.blk{2}*(var_obj.blk{2}+1); 
            idx_temp = 0.5*((idx_j_temp-1).*(idx_j_temp)) + idx_i_temp;
            info.Operator_Matrix{var_obj.block_no} = sparse(idx_temp,1:1:num_constr,value_temp,dim_temp,num_constr);
